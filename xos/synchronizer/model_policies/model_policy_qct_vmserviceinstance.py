...
class QCT_vm_ServiceInstancePolicy(TenantWithContainerPolicy):
    model_name = "QCT_vm_ServiceInstance"
    ...
    def handle_delete(self, service_instance):
        if service_instance.instance and (not service_instance.instance.deleted):
            all_service_instances_this_instance = QCT_vm_ServiceInstance.objects.filter(instance_id=service_instance.instance.id)
            ...
            if (not other_service_instances_this_instance):
                self.logger.info("QCT_vm Instance %s is now unused -- deleting" % service_instance.instance)
                self.delete_instance(service_instance, service_instance.instance)
            else:
                self.logger.info("QCT_vm Instance %s has %d other service instances attached" % (service_instance.instance, len(other_service_instances_this_instance)))
